...
<tr>
    ...
    <td class="text-end pe-3">
        @if($booking->status === 'completed')
            ...
            @endif
        @elseif(!in_array($booking->status, ['cancelled', 'in_progress', 'completed']))
            <form action="{{ route('bookings.cancel', $booking) }}" method="POST"
                  onsubmit="return confirm('{{ $booking->created_at->diffInMinutes(now()) <= 10 ? 'Cancel this booking for a 100% full refund?' : 'Cancelling after 10 minutes incurs a 10% cancellation fee (90% refund). Proceed?' }}');">
                @csrf
                @method('DELETE')
